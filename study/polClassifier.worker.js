/**
 * Given the title, url, and contents of a page, determine whether it is
 * an article, and if so, whether it is about politics. Return 0 for not an article, 
 * 1 for non-COVID article, and 2 for political article.
...
function initialize(initializationData) {
    name = initializationData.name;

    // Parse and store initialization data
    featureLabels = initializationData.args.features;
    coefficients = [initializationData.args.class0_coefs.split(",").map(i => {return parseFloat(i);}),
        initializationData.args.class1_coefs.split(",").map(i => {return parseFloat(i);}),
        initializationData.args.class2_coefs.split(",").map(i => {return parseFloat(i);})];
    intercepts = [parseFloat(initializationData.args.class0_intercept),
        parseFloat(initializationData.args.class1_intercept),
        parseFloat(initializationData.args.class2_intercept)];
    tokenizationParams = {"title": initializationData.args.title_tokenization_params,
        "content": initializationData.args.content_tokenization_params,
        "url": initializationData.args.url_tokenization_params};
    idfVector = initializationData.args.idf_vector;
}
...
function classify(pageContent) {
    const features = tfidf(getFeaturesFromPage(pageContent.url, pageContent.title, pageContent.textContent));
 ...
 * @param {string} url - URL of the page to be classified.
 * @param {string} title - Title of the page to be classified.
 * @param {string} content - Page content as parsed by the readability script.
